...
if [[ -z "$SDK_MANAGER" ]]; then
  SDK_MANAGER="$(find "$SDK/cmdline-tools" -type f -name sdkmanager 2>/dev/null | sort -V | tail -n 1)"
fi
if [[ ! -x "$SDK_MANAGER" ]]; then
  echo "Android sdkmanager not found under $SDK/cmdline-tools" >&2
  exit 1
fi
"$SDK_MANAGER" --sdk_root="$SDK" 'platform-tools' 'build-tools;35.0.0' 'platforms;android-35'

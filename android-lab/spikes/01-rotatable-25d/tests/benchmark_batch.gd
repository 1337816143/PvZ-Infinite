...
func _run() -> void:
	...
	for count: int in [0,128,512]:
		# Interleave order to avoid testing every optimized stage only after all old ones.
	...
	# Deterministic matched screenshots. All markers have identical locations in A/B.
	...
	var good: bool = soak.samples <= 600 and soak.cache_entries == 121 and soak.ground_mesh_builds == 1 and soak.node_delta == 0 and app.world.snapshot() == snapshot
	report["bounded_state_pass"] = good
	var file: FileAccess = FileAccess.open(output.path_join("report.json"),FileAccess.WRITE)
	file.store_string(JSON.stringify(report,"  "))
	file.close()
	app.free()
	print("LAB_BATCH_RENDER_OK" if good else "BATCH_FAIL: bounded state")
